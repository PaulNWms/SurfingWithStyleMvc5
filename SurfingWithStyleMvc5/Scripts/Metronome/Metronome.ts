...
module Metronome {
    ...
    class Schedule implements ISchedule {
        ...
        public drawChart(rowIndex: number) {
            ...
            let x1: number = 0;
            let y1: number = parseInt(row.children().children(".tempo-0").val());

            for (let i = 0; i <= 5; i++) {
                data.addRow([x1, y1]);
                let dur: number = parseInt(row.children().children(".midpoint-" + i + "-sec").val());
                let x2 = x1 + dur;
                let y2: number = parseInt(row.children().children(".tempo-" + (i + 1)).val());
                y2 = y2 > 0 ? y2 : y1;
                x1 = x2;
                y1 = y2;
            }
        ...
        public parseForm() {
            ...
            for (let i = 0; i < tempo0.length; i++) {
                let e: string = exercise.eq(i).val();
                let t0: number = parseInt(tempo0.eq(i).val());
                let d0: number = parseInt(midpoint0.eq(i).val());
                let t1: number = 0;
                let d1: number = 0;
                let t2: number = 0;
                let d2: number = 0;
                ...
                if (tempo1.eq(i).val()) { t1 = parseInt(tempo1.eq(i).val()); }
                if (midpoint1.eq(i).val()) { d1 = parseInt(midpoint1.eq(i).val()); }
                if (tempo2.eq(i).val()) { t2 = parseInt(tempo2.eq(i).val()); }
                if (midpoint2.eq(i).val()) { d2 = parseInt(midpoint2.eq(i).val()); }
                if (tempo3.eq(i).val()) { t3 = parseInt(tempo3.eq(i).val()); }
                if (midpoint3.eq(i).val()) { d3 = parseInt(midpoint3.eq(i).val()); }
                if (tempo4.eq(i).val()) { t4 = parseInt(tempo4.eq(i).val()); }
                if (midpoint4.eq(i).val()) { d4 = parseInt(midpoint4.eq(i).val()); }
                if (tempo5.eq(i).val()) { t5 = parseInt(tempo5.eq(i).val()); }
                if (midpoint5.eq(i).val()) { d5 = parseInt(midpoint5.eq(i).val()); }
        ...
        public preset4Button() {
            ...
            let s: number = row.children().children(".duration-sec").val();
